...
- **long:** Ακέραιοι αριθμοί μεγάλου εύρους, Το εύρος του long στην C εξαρτάται από το σύστημα, αλλά στις περισσότερες περιπτώσεις είναι -2³¹ έως 2³¹-1 (για 32-bit) ή -2⁶³ έως 2⁶³-1 (για 64-bit).
...
- **Λευκό (#FFFFFF)**: Συνδυασμός όλων των χρωμάτων σε πλήρη ένταση.
...
public class VowelsCount {
    public static void main(String[] args) {
        ...
        // Ζητάμε από το χρήστη να εισάγει μια φράση
...
- One-to-One: Μία εγγραφή αντιστοιχεί σε μία μόνο εγγραφή.
- One-to-Many: Μία εγγραφή αντιστοιχεί σε πολλές εγγραφές.
- Many-to-Many: Πολλές εγγραφές αντιστοιχούν σε πολλές εγγραφές.
...
```php
<?php
echo "<i>" . $_POST['text1'] . "</i><br>";
echo "<i>" . $_POST['text2'] . "</i>";
?>
```
...
<form action="process.php" method="post">
    ...
    Επίθετο: <input type="text" name="surname"><br>
    Κωδικός: <input type="text" name="code"><br>
    <input type="submit" value="Submit">
</form>
```
**Στο process.php**:
...
if (empty($_POST['name']) || empty($_POST['surname']) || empty($_POST['code'])) {
    echo "<p style='color:red;'>Πρέπει να συμπληρώσετε όλα τα πεδία!</p>";
    include('form.html');
} else {
    echo "Όνομα: " . $_POST['name'] . "<br>";
    echo "Επίθετο: " . $_POST['surname'] . "<br>";
    echo "Κωδικός: " . $_POST['code'];
}
?>
```
...
```html
<form action="display_variable.php" method="post">
    <input type="text" name="variable">
    <input type="submit" value="Submit">
</form>
```
**Στο display_variable.php**:
```php
<?php
$var1 = 10;
$var2 = 20;
...
$var = $_POST['variable'];
echo $$var;  // Εμφανίζει την τιμή της μεταβλητής που αντιστοιχεί στο όνομα από το πεδίο
?>
```

## ✅86. Ποιες είναι οι βασικές διαφορές μεταξύ PHP και JAVASCRIPT;

**PHP**: Server-side scripting, εκτελείται στον server.
**JavaScript**: Client-side scripting, εκτελείται στον browser.

## ✅87. Πώς μπορούμε να έχουμε καθολικές μεταβλητές μεταξύ πολλώνσυναρτήσεων; Δώστε από ένα παράδειγμα σε κάθε περίπτωση.

**PHP**: Χρήση global ή $GLOBALS. **Παράδειγμα**:
...
test();
```
**JavaScript**: Χρήση window για καθολικές μεταβλητές. *Παράδειγμα*:
```javascript
var globalVar = 5;
function test() {
...
Δεν μπορείς να ανοίξεις ένα αρχείο PHP με διαχειριστή αρχείων γιατί χρειάζεται web server (π.χ. Apache) για να εκτελέσει τις PHP εντολές.
...
<form action="calculate.php" method="post">
    Αριθμός 1: <input type="number" name="num1"><br>
    Αριθμός 2: <input type="number" name="num2"><br>
    Πράξη:
...
<form action="circle_area.php" method="post">
    Ακτίνα: <input type="number" name="radius"><br>
...
if ($_POST['radius']) {
    $radius = $_POST['radius'];
    echo "Εμβαδόν = " . calculateArea($radius);
}
?>
```
...
Για την αποθήκευση και ανάκτηση δεδομένων μεταξύ διαφορετικών σελίδων.

**Παράδειγμα 1**: Αποθήκευση όνομα χρήστη
```php
session_start();
$_SESSION['username'] = "user123";
```
**Παράδειγμα 2**: Έλεγχος εάν ο χρήστης είναι συνδεδεμένος.
```php
session_start();
...
```php
<?php
// Για κάθε εγγραφή στον πίνακα subscribers
foreach ($subscribers as $subscriber) {
    $to = $subscriber['email'];
    $subject = "Login info";
    $body = "Your username is " . $subscriber['username'] . " and your password is " . $subscriber['password'];
...
Ο όρος **static mesh** αναφέρεται σε ένα 3D μοντέλο που παραμένει αμετάβλητο κατά τη διάρκεια του παιχνιδιού και δεν επηρεάζεται από φυσικές δυνάμεις, όπως βαρύτητα ή συγκρούσεις. 

### Σκοπός:
- **Ακίνητο:** Τα **static meshes** δεν κινούνται ή παραμορφώνονται κατά τη διάρκεια του παιχνιδιού.
- **Απόδοση:** Είναι βελτιστοποιημένα για καλύτερη απόδοση, καθώς δεν απαιτούν υπολογισμούς φυσικής.
- **Χρησιμοποιούνται για:** Σκηνικά αντικείμενα, τοίχους, έπιπλα και άλλα στοιχεία του περιβάλλοντος.

## ✅100. Τι είναι τα collision boxes στα Game Engines; Τι εξυπηρετούν και με ποιο σκεπτικό τα κατασκευάζουμε;

Τα **collision boxes** στα Game Engines είναι γεωμετρικά σχήματα (όπως κύβοι ή σφαίρες) που χρησιμοποιούνται για να ανιχνεύσουν συγκρούσεις μεταξύ αντικειμένων στον κόσμο του παιχνιδιού. 

### Εξυπηρετούν:
1. **Ανίχνευση Συγκρούσεων:** Ανιχνεύουν πότε τα αντικείμενα συγκρούονται.
2. **Φυσική Παγίδευση:** Καθορίζουν τις αντιδράσεις των αντικειμένων σε συγκρούσεις.
3. **Αποδοτικότητα:** Εξοικονομούν υπολογιστικούς πόρους με τη χρήση απλών σχημάτων.

### Σκεπτικό Κατασκευής:
1. **Απλότητα και Απόδοση:** Χρησιμοποιούμε απλά σχήματα για γρήγορη ανίχνευση.
2. **Προσαρμογή:** Επιλέγουμε το κατάλληλο σχήμα ανάλογα με το αντικείμενο.
3. **Ευκολία Υπολογισμού:** Επιλέγουμε σχήματα με εύκολους μαθηματικούς υπολογισμούς για ταχύτητα.

## ✅101. Αναφέρετε ποιες τεχνικές φωτισμού χρησιμοποιούνται στα video games και ποια τα χαρακτηριστικά τους.

- **Ambient Lighting:** Γενικό φως χωρίς συγκεκριμένη κατεύθυνση, δημιουργεί μαλακή ατμόσφαιρα.
- **Directional Lighting:** Προσομοιώνει την ηλιακή ακτινοβολία με κατεύθυνση, κατάλληλο για εξωτερικούς χώρους.
- **Point Lighting:** Φως που εκπέμπεται από μία καθορισμένη θέση (π.χ. λαμπτήρας), εκπέμπεται σε όλες τις κατευθύνσεις.
- **Spot Lighting:** Κώνος φωτός που εκπέμπεται από ένα σημείο προς μια συγκεκριμένη περιοχή.
- **Global Illumination (GI):** Φωτισμός που υπολογίζει τη διάδοση του φωτός από τα αντικείμενα στο περιβάλλον.

## ✅102. Τι είναι το bill boarding και ποια τα πλεονεκτήματά του στην κατασκευή 3D games;

- **Ορισμός:** Μία τεχνική όπου οι 2D εικόνες (sprites) παραμένουν στραμμένες προς την κάμερα, ανεξάρτητα από τη θέση της, για να φαίνονται τρισδιάστατες.
- **Πλεονεκτήματα:** Εξοικονόμηση πόρων και απόδοσης, κατάλληλο για τη δημιουργία αντικειμένων με πολλαπλά επίπεδα χωρίς την ανάγκη 3D μοντέλων (π.χ. δέντρα, σύννεφα).
...
- **Ορισμός:** Τεχνική που χρησιμοποιεί μοντέλα με διαφορετικά επίπεδα ανάλυσης ανάλογα με την απόσταση από την κάμερα.
- **Χρησιμότητα:** Βελτιώνει την απόδοση του παιχνιδιού μειώνοντας τον υπολογιστικό φόρτο για αντικείμενα που είναι μακριά από την κάμερα.

## ✅104. Να αναφερθούν συνοπτικά οι βασικές κλάσεις του Unreal Editor και ποια η λογική της  χρησιμοποίησής τους.

- **Actor:** Ουσιαστικά κάθε αντικείμενο στο παιχνίδι (π.χ. χαρακτήρες, αντικείμενα).
- **Pawn:** Ειδικός τύπος Actor που μπορεί να ελεγχθεί από τον παίκτη ή την AI.

## ✅105. Περιγράψτε τη κλάση actor (στην UnrealScript), αναφέρατε τη χρησιμότητά της.

- **Ορισμός:** Ο βασικός τύπος αντικειμένου στον κόσμο του παιχνιδιού, που μπορεί να έχει γεωμετρία, φυσική και συμπεριφορά.
- **Χρησιμότητα:** Χρησιμοποιείται για να περιγράψει οποιοδήποτε αντικείμενο με το οποίο μπορεί να αλληλεπιδράσει ο παίκτης ή το περιβάλλον.

## ✅106. Περιγράψτε τη κλάση pawn (στην UnrealScript), αναφέρατε τη χρησιμότητά της.

- **Ορισμός:** Ειδικός τύπος Actor που μπορεί να ελεγχθεί από τον παίκτη ή την AI.
- **Χρησιμότητα:** Χρησιμοποιείται για τον έλεγχο χαρακτήρων ή άλλων αντικειμένων που αλληλεπιδρούν άμεσα με τον παίκτη ή την AI.
...
- **string:** Κείμενα.
- **vector:** Τρεις αριθμοί για θέση, κατεύθυνση ή χρώμα.
...
- **Equipped:** Το όπλο είναι έτοιμο για χρήση.
- **Firing:** Το όπλο πυροβολεί.
- **Reloading:** Το όπλο επαναφορτίζεται.
- **Out of Ammo:** Το όπλο δεν έχει σφαίρες.

## ✅109. Τι σημαίνουν οι όροι Modeling, Rendering και Animation;

- **Modeling:** Δημιουργία τρισδιάστατων μοντέλων (π.χ. χαρακτήρων, αντικειμένων).
- **Rendering:** Η διαδικασία απόδοσης τρισδιάστατων μοντέλων σε 2D εικόνα ή βίντεο.
- **Animation:** Δημιουργία κίνησης για τα 3D μοντέλα, όπως για παράδειγμα κινούμενα σχέδια.

## ✅110. Τι είναι φωτοαπόδοση και ποιοι παράγοντες την επηρεάζουν;

- **Ορισμός:** Η διαδικασία της δημιουργίας εικόνας που μιμείται την πραγματική φωτογραφία μέσω υπολογιστικών μεθόδων.
- **Παράγοντες που την επηρεάζουν:** 
  - Φωτισμός
  - Υλικά και υφές
  - Αλληλεπιδράσεις μεταξύ φωτός και επιφανειών
  - Γωνία θέασης και προοπτική
  - Σκίαση και ανάλυση εικόνας
...
1. **Σχεδίαση μοντέλων (Modeling):** Δημιουργία των τρισδιάστατων αντικειμένων.
2. **Εφαρμογή υλικών και υφών (Materials & Textures):** Προσθήκη λεπτομερειών στην επιφάνεια των μοντέλων.
3. **Ρύθμιση φωτισμού (Lighting):** Προσαρμογή του φωτισμού για φωτορεαλισμό.
4. **Ρύθμιση κάμερας (Camera Setup):** Επιλογή γωνίας και απόστασης λήψης.
5. **Animation (Κίνηση):** Δημιουργία της κίνησης για τα αντικείμενα.
6. **Rendering:** Δημιουργία της τελικής εικόνας ή βίντεο.
...
- **Φωτισμός (Lighting):** Τεχνικές που καθορίζουν πώς το φως αλληλεπιδρά με τα αντικείμενα.
  - **Point Lighting:** Φως από ένα συγκεκριμένο σημείο.
  - **Directional Lighting:** Φως από μία κατεύθυνση.
  - **Spot Lighting:** Κώνος φωτός που εκτείνεται σε συγκεκριμένη περιοχή.
  - **Ambient Lighting:** Γενικό φως χωρίς καθορισμένη κατεύθυνση.
  - **Σκίαση (Shading):** Τεχνικές που καθορίζουν πώς η επιφάνεια ενός αντικειμένου αντανακλά το φως.
  - **Flat Shading:** Εφαρμογή μιας ενιαίας σκιάς σε όλη την επιφάνεια.
  - **Gouraud Shading:** Αντιστοίχιση χρώματος σε κάθε κορυφή και ενδιάμεση σκίαση.
  - **Phong Shading:** Σκίαση κάθε pixel με πιο ακριβή υπολογισμό του φωτισμού.

## ✅113. Ποιες τεχνικές γνωρίζετε για την απόδοση υλικών (materials & textures) πάνω στα τρισδιάστατα μοντέλα;

- **UV Mapping:** Τεχνική τοποθέτησης υφών σε 3D μοντέλα.
- **Normal Mapping:** Δημιουργία της ψευδαίσθησης ανατομίας ή λεπτομέρειας χωρίς επιπλέον πολύγωνα.
- **Bump Mapping:** Αντίστοιχη ψευδαίσθηση ανατομίας αλλά μέσω διαφορών στις υφές.
- **Reflection Mapping:** Δημιουργία αντανακλάσεων σε επιφάνειες.

## ✅114. Τι είναι το Normal Mapping και ποια η χρησιμότητα του; Περιγράψτε αναλυτικά τον τρόπο λειτουργίας του.

- **Ορισμός:** Τεχνική που χρησιμοποιεί ειδικούς χάρτες κατευθύνσεων για να προσδώσει ψευδαίσθηση ανατομίας στις επιφάνειες.
- **Χρησιμότητα:** Δίνει την αίσθηση λεπτομερειών και επιφανειακών χαρακτηριστικών χωρίς να προσθέτει περισσότερα πολύγωνα στο μοντέλο.
- **Λειτουργία:** Η υφή κατευθύνσεων που εφαρμόζεται στην επιφάνεια τροποποιεί τη σκίαση της και δημιουργεί την ψευδαίσθηση περισσότερης λεπτομέρειας.
...
- **Ορισμός:** Τεχνικές που χρησιμοποιούνται για τη δημιουργία και απόδοση της τελικής εικόνας σε 3D γραφικά.
- **Τρεις βασικές τεχνικές rendering:**
  - **Ray Tracing:** Προσομοιώνει την κίνηση των ακτίνων φωτός για πιο ακριβή φωτισμό και σκιές.
  - **Rasterization:** Τεχνική απόδοσης όπου τα τρίγωνα μετατρέπονται σε pixels.
  - **Path Tracing:** Αναπαράγει τον τρόπο με τον οποίο το φως διαδίδεται στο περιβάλλον με πιο ρεαλιστικά αποτελέσματα.
...
- **Απαραίτητος φωτισμός:** Ο φωτισμός είναι κρίσιμος για τη δημιουργία ρεαλισμού και βάθους στις σκηνές 3D, καθώς βοηθά στην απόδοση των χρωμάτων και τη δημιουργία σκιών που ενισχύουν την αίσθηση της πραγματικότητας.
- **Τρεις τρόποι φωτισμού:**
  - **Point Light:** Φως που προέρχεται από ένα συγκεκριμένο σημείο και διαχέεται σε όλες τις κατευθύνσεις (π.χ., λάμπα).
  - **Directional Light:** Φως που έρχεται από μια συγκεκριμένη κατεύθυνση, όπως η ηλιακή ακτίνα.
  - **Ambient Light:** Γενικός φωτισμός που διαχέεται ομοιόμορφα σε όλη τη σκηνή χωρίς να προέρχεται από συγκεκριμένη κατεύθυνση.

## ✅117. Τι είναι τα function curves (στα προγράμματα τρισδιάτατης απεικόνισης) και που χρησιμεύουν;

- **Ορισμός:** Τα function curves (καμπύλες συναρτήσεων) χρησιμοποιούνται για την αναπαράσταση της αλλαγής κάποιου παραμέτρου με την πάροδο του χρόνου (π.χ. κίνηση, φωτισμός, κ.ά.).
- **Χρησιμότητα:** Χρησιμοποιούνται για την ρύθμιση παραμέτρων όπως η ταχύτητα κίνησης ενός αντικειμένου, η ένταση φωτισμού ή η αλλαγή του χρώματος μέσα στο χρόνο.

## ✅118. Τι είναι το UV Mapping;

- **Ορισμός:** Η τεχνική του UV Mapping αναφέρεται στην τοποθέτηση μιας 2D υφής πάνω σε ένα 3D μοντέλο. Οι συντεταγμένες UV χρησιμοποιούνται για να «κολλήσουν» την υφή στο μοντέλο, προσδιορίζοντας ποιο μέρος της υφής αντιστοιχεί σε ποιο σημείο του 3D μοντέλου.


## ✅119. Τι είναι το Diffuse Map και ποια η διαφορά του από το Texture Map;

- **Diffuse Map:** Είναι η υφή που καθορίζει το βασικό χρώμα της επιφάνειας ενός 3D αντικειμένου χωρίς να επηρεάζει την αντανάκλαση ή άλλες παραμέτρους φωτισμού.
- **Διαφορά από το Texture Map:** Το **Diffuse Map** είναι ένα είδος **Texture Map** που επηρεάζει το χρώμα του μοντέλου, ενώ το **Texture Map** γενικότερα μπορεί να αναφέρεται σε οποιοδήποτε είδος υφής (π.χ. κανονικό χάρτη ή χάρτη αντανάκλασης).
...
**Ορισμός:** Το Opacity Map είναι μια υφή που καθορίζει το βαθμό διαφάνειας ή αδιαφάνειας μιας επιφάνειας σε ένα 3D μοντέλο. Χρησιμοποιείται για να δημιουργήσει εφέ διαφάνειας, όπως τζάμια, υγρά ή άλλες μερικώς διαφανείς επιφάνειες.
...
public class MovingBall extends JPanel {
    private int x = 100, y = 100; // Αρχική θέση του μπαλακιού
...
- **Αύξηση Εμπιστοσύνης:** Οι χρήστες εμπιστεύονται περισσότερο τις ιστοσελίδες που βρίσκονται στις πρώτες θέσεις των αποτελεσμάτων αναζήτησης.
...
1. **Component**:
   - Μεγάλη μονάδα λειτουργικότητας που μπορεί να περιλαμβάνει πλήρη χαρακτηριστικά.
...
Σε μια ιστοσελίδα υπάρχουν συνήθως οι παρακάτω τύποι χρηστών:

1. **Διαχειριστής (Admin)**:
   - Έχει πλήρη πρόσβαση σε όλες τις λειτουργίες της ιστοσελίδας, όπως η δημιουργία, επεξεργασία και διαγραφή περιεχομένου, η διαχείριση χρηστών, κ.λπ.
...
Τα βασικά μέρη του ψηφιακού βίντεο είναι:
...
Η διαδικασία μοντάζ για την παραγωγή ενός ψηφιακού αρχείου video συνήθως περιλαμβάνει τα εξής τυπικά βήματα:
...
- Βάθος χρώματος: 16 bit (2 byte)
- Ρυθμός καρέ: 25 καρέ ανά δευτερόλεπτο (για PAL)
- Διάρκεια: 2 λεπτά = 120 δευτερόλεπτα

\[
\text{Μέγεθος αρχείου} = 320 \times 240 \times 2 \times 25 \times 120
\]

Ας υπολογίσουμε το αποτέλεσμα:
...
Στο Flash υπάρχουν τρεις βασικοί τύποι symbols:
...
Τα πεδία του **classic text** στο Flash περιλαμβάνουν:
...
Αυτό σημαίνει ότι το μπλε τετράγωνο θα βρίσκεται στο κέντρο του stage, με 200px κενό από την αριστερή πλευρά του και 300px από την επάνω πλευρά του, αφήνοντας 200px κενό στην κάτω και δεξιά πλευρά του.

## ✅162. Τι είναι τα code snippets;

Τα **code snippets** είναι προκαθορισμένα, μικρά τμήματα κώδικα που μπορούμε να εισάγουμε σε ένα έργο Flash για να προσθέσουμε λειτουργικότητα. Αυτά τα κομμάτια κώδικα μπορεί να αφορούν διάφορες ενέργειες, όπως η μετακίνηση ενός αντικειμένου, η διαχείριση του ήχου, η δημιουργία αλληλεπιδράσεων με το χρήστη ή η αλλαγή σκηνών. Τα snippets προσφέρουν γρήγορες λύσεις για συχνά χρησιμοποιούμενες ενέργειες, χωρίς να χρειάζεται να γράφουμε τον κώδικα από την αρχή.
...
1. **Δημιουργία του Path**:
   - Σχεδιάζουμε το μονοπάτι (path) που θέλουμε να ακολουθήσει το αντικείμενο με το εργαλείο **Pen Tool** ή **Line Tool**. Αυτό το μονοπάτι θα είναι το path animation.
